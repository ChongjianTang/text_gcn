...
from gcn import GCN  # GCN网络
from mlp import MLP  # 全连接网络

from config import CONFIG  # 模型配置信息
...
cfg.dataset = dataset  # 用命令行设置的数据集 对配置对象中的数据集进行覆盖
...
# featureless (train_size + vocab_size+test_size, train_size + vocab_size+test_size) 对角线为1
# 每个文档和单词表示为one-hot
...
if cfg.model == 'gcn':
    support = [preprocess_adj(adj)]  # 归一化、引入自连接的邻接矩阵
    num_supports = 1
    model_func = GCN
    ...
    model_func = GCN
elif cfg.model == 'dense':  # 全连接网络 不需要使用邻接矩阵
...
# tm_train_mask（train_size+vocab_size+test_size,num_classes） 前real_train_size行全为1
...
def evaluate(features, labels, mask):
    ...
    model.eval()
    with torch.no_grad():
        logits = model(features)  # 得到全体数据集的输出 (train_size+vocab_size+test_size,num_classes)
        t_mask = torch.from_numpy(np.array(mask * 1., dtype=np.float32))
        tm_mask = torch.transpose(torch.unsqueeze(t_mask, 0), 1, 0).repeat(1, labels.shape[1])
        loss = criterion(logits * tm_mask, torch.max(labels, 1)[1])  # 选出验证集对应的输出 以及 验证集对应的标签 计算loss
        pred = torch.max(logits, 1)[1]  # 计算在验证集上预测的 标签
        # 计算模型在验证集上的准确率
...
# Train model
for epoch in range(cfg.epochs):  # full-batch 训练 一个epoch做一次更新

    t = time.time()

    # Forward pass
    logits = model(t_features)  # (train_size+vocab_size+test_size,num_classes)
    loss = criterion(logits * tm_train_mask, torch.max(t_y_train, 1)[1])  # 选出真实训练集对应的输出 和 真实训练集对应的标签 计算loss
    # 计算真实训练集上的准确率
    ...
    if epoch > cfg.early_stopping and val_losses[-1] > np.mean(val_losses[-(cfg.early_stopping + 1):-1]):  # 提前终止
        print_log("Early stopping...")
        break

print_log("Optimization Finished!")

# Testing  训练结束后 计算模型在测试集上的损失、准确率 预测的标签
...
# doc and word embeddings 提取文档和词(节点)的嵌入 方便后续进行降维可视化
